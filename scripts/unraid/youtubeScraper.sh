...
then
    ...
    do
        ...
        --match-filter "original_url!*=/shorts/ & !is_live & !was_live & availability!=premium_only" --no-progress \
        --cache-dir /workdir/.cache --datebefore $oneMonthAgo --merge-output-format mp4 -o "$outputFormat" "$url"
